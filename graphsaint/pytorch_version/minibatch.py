...
class Minibatch:
    ...
    def __init__(self, adj_full_norm, adj_train, role, train_params, cpu_eval=False):
        ...
        _denom = len(self.node_train) + \
            len(self.node_val) + len(self.node_test)
    ...
    def set_sampler(self, train_phases):
        ...
        if self.method_sample == 'mrw':
            if 'deg_clip' in train_phases:
                ...
                # setting this to a large number so essentially there is no clipping in probability
                _deg_clip = 100000
            ...
            self.size_subg_budget = train_phases['num_root'] * \
                train_phases['depth']
    ...
    def one_batch(self, mode='train'):
        ...
        if mode in ['val', 'test', 'valtest']:
            ...
            if self.use_cuda:
                adj = adj.cuda()
            self.batch_num += 1
        norm_loss = self.norm_loss_test if mode in [
            'val', 'test', 'valtest'] else self.norm_loss_train
